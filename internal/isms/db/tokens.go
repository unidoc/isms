...
func (d *DB) GetUserByID(ctx context.Context, id int) (*User, error) {
	...
	`, id).Scan(&u.ID, &u.Email, &u.Name, &u.PasswordHash, &u.OTPSecret, &u.OTPVerified, &u.EmailVerified, &u.PendingEmail, &u.IsAgent, &u.Active, &u.Locale, &u.CreatedAt, &u.LastSeen)
	if err != nil {
		return nil, err
	}
