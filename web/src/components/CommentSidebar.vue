  ...
  <div class="flex flex-col h-full">
    ...
    <button @click="expanded = !expanded"
      ...
      <div class="flex items-center gap-2">
        ...
        <h3 class="text-sm font-semibold text-slate-300">{{ $t('components.comment_sidebar.title') }}</h3>
      </div>
      <div class="flex items-center gap-2">
        <span v-if="openCount > 0" class="text-xs font-bold bg-blue-600 text-white px-1.5 py-0.5 rounded-full">{{ openCount }}</span>
        <span v-if="resolvedCount > 0" class="text-xs text-slate-600">{{ $t('components.comment_sidebar.resolved_count', resolvedCount) }}</span>
      </div>
    </button>
    ...
    <template v-if="expanded">
      ...
      <div class="px-5 py-4 border-b border-slate-800 space-y-3 flex-shrink-0">
        <MentionTextarea
          v-model="newComment"
          :members="members"
          :placeholder="placeholder || $t('components.comment_sidebar.comment_placeholder')"
          rows="3"
        ...
        <div class="flex items-center gap-2">
          <input
            v-model="author"
            :placeholder="$t('components.comment_sidebar.name_placeholder')"
          ...
          >
            {{ submitting ? '...' : $t('components.comment_sidebar.send') }}
          </button>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        <div v-if="loading" class="p-5 text-xs text-slate-600">{{ $t('common.state.loading') }}</div>
        <div v-else-if="comments.length === 0" class="p-5 text-xs text-slate-600">{{ $t('components.comment_sidebar.empty') }}</div>
        <template v-else>
          <!-- Open comments -->
          <div v-for="c in openComments" :key="c.id" class="px-5 py-4 border-b border-slate-800">
            ...
            <div v-if="replyingTo === c.id" class="mt-2 ml-4 pl-3 border-l-2 border-blue-600">
              <MentionTextarea
                v-model="replyText"
                :members="members"
                :placeholder="$t('components.comment_sidebar.reply_placeholder')"
                rows="2"
          ...
          <button v-if="resolvedCount > 0" @click="showResolved = !showResolved"
            ...
            {{ $t('components.comment_sidebar.resolved_count', resolvedCount) }}
          </button>
